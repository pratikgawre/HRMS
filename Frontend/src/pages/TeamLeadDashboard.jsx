...
function TeamLeadDashboard() {
  ...
  useEffect(() => {
    ...
    const refreshScope = () => {
      ...
      ]).then(([employeeRows, projectRows, taskRows, announcementRows]) => {
        if (!active) {
          return;
        }
  ...
  const teamLeadStats = [
    ...
    },
    {
      ...
      value: String(visibleLeaves.length).padStart(2, '0'),
      delta: `${pendingLeaveCount} awaiting action`,
      tone: 'pink',
  ...
  const quickActionDetails = {
    ...
    'Approve Leave': `${visibleLeaves.length} total`,
  ...
  return (
    <>
      ...
      <div className="dashboard-grid team-lead-section-grid">
        ...
        <Section title="Leave Review" action="Review" actionOnClick={() => navigate('/team-lead/leave-review')}>
